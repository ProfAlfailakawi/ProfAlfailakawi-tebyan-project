...
export async function generatePodcastScript(questionData: any): Promise<string> {
  return withRetry(async () => {
    const prompt = `
    أنتِ مستشارة كويتية فاضلة، حكيمة، ودافئة المشاعر. تكتبين "سوالف وتأملات" (Podcast Script) تدخل القلب.
    المهمة: تحويل هذا الموقف أو الاستشارة إلى خاطرة صوتية بلهجة كويتية بيضاء جميلة، تحكينها كإنسانة خبيرة وحكيمة لمستمعيك.
    ...
    الأسلوب المطلوب (ضروري جداً):
    1. **اللهجة:** استخدمي "اللهجة الكويتية البيضاء" البسيطة والمفهومة للجميع. نريدها عفوية جداً، طبيعية وغير روبوتية، وكأنكِ قاعدة تسولفين معاهم بصدق وبدون تكلف.
    2. **نمط السرد:** النص عبارة عن "خاطرة" أو "نصيحة من القلب". يمكنك إدخال مقطوعات من حوار قصير داخل سردك (مثلاً: "بنتي تسألني يما ليش جذي؟ أقول لها يا بنيتي...").
    3. **الروح:** النص لازم يكون حنون جداً ومطمئن، استخدمي كلمات كويتية دافئة، اجعليه يشعر المستمع أنه يستمع لأم أو مستشارة محبة.
    4. **الصياغة:** ابتعدي تماماً عن الفصحى الجافة أو السرد المعقم. اكتبيه بطريقة تناسب الأداء الصوتي الحيوي.
    5. **الطول:** مكثف جداً وواضح.
    `;
