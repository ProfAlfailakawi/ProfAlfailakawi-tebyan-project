...
> **آخر تحديث:** البندان 1 و 2 عولجا في الكود (راجع تفاصيلهما أدناه). البند 1 يحتاج **خطوة
> إعداد من المالك** قبل نشر الدالة وإلا سترجع نقاط الذكاء الاصطناعي 401. البند 3 **قرار
> صريح للمالك بعدم التغيير** — لا تمسّه. بقيّة البنود ما زالت مفتوحة.
...
- ما صار:
  ...
  2. **App Check صار مُفعَّلاً افتراضياً في الإنتاج** ومعطّلاً في التطوير/المحاكي.
     - `APP_CHECK_ENFORCE=true` → إجبار التفعيل في أي بيئة.
     - `APP_CHECK_ENFORCE=false` → إطفاء، **ويُحترم خارج الإنتاج فقط** حتى لا يُعيد متغيّر
       بيئة شارد فتحَ الوكيل على الموقع الحيّ.
     - `APP_CHECK_MONITOR_ONLY=true` → يتحقّق ويسجّل لكن يخدم الطلب. هذا مفتاح **الطرح
       المرحلي** وهو السبيل الوحيد لتخفيف الإجبار في الإنتاج.
