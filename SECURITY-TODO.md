...
## [مُطبَّق] حارس تصعيد الصلاحيات في users
طُبِّق في `firestore.rules` (كتلة `/users/{userId}`): يمنع المستخدم من منح نفسه `role:'admin'`
أو تغيير دوره عبر العميل. إضافة بحتة لا تكسر أي مسار عامل (إدارة الأدمن لمستخدمين آخرين
كانت أصلاً محجوبة بشرط `request.auth.uid == userId`).
تنبيه: `DRAFT_firestore.rules` **ليس بديلاً آمناً** — يحذف default-deny وبوابات الأدمن؛ لا يُنشر كما هو.
يبقى للنشر الفعلي: `firebase deploy --only firestore:rules`.
