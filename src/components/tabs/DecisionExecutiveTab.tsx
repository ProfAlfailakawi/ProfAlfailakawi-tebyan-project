...
export const DecisionExecutiveTab = ({ language, handleTabChange, initialValue = '', onValueUsed }: { language: 'ar' | 'en', handleTabChange: any, initialValue?: string, onValueUsed?: () => void }) => {
    ...
    const tools: AnalysisTool[] = [
        ...
        { id: 'memory', title: { ar: 'البصمة الفكرية', en: 'Cognitive Trace' }, icon: History, bgColor: 'bg-blue-50 text-blue-700 border border-blue-200' , purpose: ['voices'], hint: { ar: 'يربط القرار بسياق التفكير', en: 'Links the decision to thinking patterns' }, prompt: 'حلل هذا القرار بناءً على سياق التفكير الاستراتيجي العميق ومبادئه: ' },
    ...
    const selectDecisionPurpose = (purposeId: string) => {
        ...
        <div className="space-y-6 px-2 pb-20 max-w-6xl mx-auto" dir={language === 'ar' ? 'rtl' : 'ltr'}>
           <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
             <div className="space-y-4">
                ...
                <h1 className="text-3xl md:text-5xl font-black text-[#182231] tracking-tight leading-none uppercase">
           ...
           <div className="relative group/input">
             ...
             <textarea 
               ...
               className={cn(
                 "w-full min-h-[132px] md:min-h-[150px] h-32 md:h-36 p-5 md:p-6 pt-9 rounded-[26px] md:rounded-[34px] border text-base md:text-lg font-medium focus:ring-8 transition-all relative z-10 custom-scrollbar resize-none leading-relaxed",
           ...
           <div className="bg-[#FAF9F6]/92 border border-[#8FA9C7]/15 rounded-[28px] md:rounded-[32px] p-4 md:p-6 space-y-4 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                >
                  <div>
                    <div className="font-black text-sm md:text-base text-[#182231]">{language === 'ar' ? activePurpose.title.ar : activePurpose.title.en}</div>
                    <div className="text-[11px] md:text-xs font-bold text-[#64788D] mt-0.5">{language === 'ar' ? activePurpose.hint.ar : activePurpose.hint.en}</div>
                  </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
                {visibleDecisionTools.map(tool => (
                    <button 
                      ...
                      className={cn(
                          "group relative flex flex-col items-center justify-center gap-2.5 md:gap-4 p-3.5 md:p-5 rounded-[20px] md:rounded-[28px] tebyan-tool-card bg-[#FAF9F6]/88 border border-[#8FA9C7]/15 shadow-sm transition-all hover:shadow-[0_15px_40px_rgba(0,0,0,0.05)] hover:border-[#8E7AAE] active:scale-[0.96] overflow-hidden",
                    ...
                    >
                        ...
                        <span className="font-black text-center text-xs md:text-sm px-1">{language === 'ar' ? tool.title.ar : tool.title.en}</span>
           ...
           <div id="decision-results" className="scroll-mt-24">
             <AnimatePresence>
              {isLoading && (
