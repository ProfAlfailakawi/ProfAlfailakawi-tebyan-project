    ...
    return (
        <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed right-0 top-0 h-full w-80 bg-white shadow-2xl z-50 p-6 overflow-y-auto">
            ...
            <div className="space-y-4">
               ...
               <button onClick={() => handleTabChange('MindMapTab', ideas[0]?.text)} className="w-full bg-zinc-100 text-black p-3 rounded-lg flex items-center gap-2 text-sm"><BrainCircuit size={16}/> {language === 'ar' ? 'التكثيف المعرفي' : 'AI Mind-Mapping'}</button>
            </div>
