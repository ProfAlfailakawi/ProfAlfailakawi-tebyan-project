...
const HubCard: React.FC<{
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={cn(
        "group ceramic-card knowledge-tectonic glass-panel rounded-[32px] md:rounded-[40px] p-6 md:p-8 border transition-all duration-500 flex flex-col justify-between relative overflow-hidden",
        inverted ? "bg-zinc-950/80 border-zinc-800 hover:shadow-[0_20px_60px_rgba(255,255,255,0.08)] hover:border-zinc-600" : "bg-white/70 border-zinc-100/80 hover:shadow-[0_20px_60px_rgba(226,192,137,0.12)] hover:border-zinc-200"
      )}
...
export const HomeDashboard = ({ tabs, handleTabChange, language }: { tabs: any[], handleTabChange: (id: string) => void, language: string }) => {
  ...
  return (
    <div className="w-full flex flex-col pt-4 pb-16 md:pt-6 px-4 md:px-6 max-w-6xl mx-auto space-y-5">
      ...
      <section className="bg-white/70 backdrop-blur-md border border-zinc-100/80 rounded-[28px] md:rounded-[40px] p-5 md:p-8 shadow-[0_12px_50px_rgba(0,0,0,0.04)] glass-panel ceramic-card" dir={language === 'ar' ? 'rtl' : 'ltr'}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <button
            ...
            className="shrink-0 px-6 py-4 rounded-2xl bg-mood-primary text-black font-black text-sm hover:opacity-90 active:scale-95 transition-all shadow-lg"
          >
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="flex-1 w-full">
                  <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <h3 className="font-black text-black tracking-widest uppercase text-[10px] md:text-xs">{language === 'ar' ? 'التحدي الميداني اليومي' : 'Daily Challenge'}</h3>
                  </div>
                  {missionLoading ? (
