...
export default function CustomerSite() {
  ...
  const moodPlaceholders = useMemo(() => [
    "كيف هو مزاجك اليوم؟ أم لديك مناسبة؟ اكتب وسنساعدك بالحال! 👨‍🍳",
    "ابنتي مريضة وأريد شيئاً دافئاً ومريحاً.. 🤍",
    "عدنا للتو من السفر والمنزل فارغ ونشعر بجوع شديد.. ✈️",
    "ما الذي تفضله اليوم؟ اكتب ما ترغب به وسنلبي طلبك 🎯",
    "أتشعر بالجوع والحيرة؟ أخبرني برغبتك وسأقوم بمساعدتك 🚀",
    "هل استقبلت ضيوفاً فجأة؟ لا تقلق، الحل لدي، فقط اكتب طلبك 🏃‍♂️",
    "هل تشتهي طبقاً معيناً؟ لا تبحث.. اكتب وسأحضره لك 🌟",
    "أتريد وجبة خفيفة أم دسمة؟ أخبرني برغبتك 🍔"
  ], []);
    ...
    if (!sessionStorage.getItem("flashSaleSeen") && topProducts.length > 0) {
      const t = setTimeout(() => {
        const phrases = [
          "مذاق رائع، جربه الآن!",
          "يستحق التجربة اليوم!",
          "المذاق الأصيل الذي يعدل المزاج!",
          "ماذا تفضل أن نأكل اليوم؟",
        ];
        const randomPhrase =
    ...
    const handleScroll = () => {
      ...
      if (
        ...
        if (listToUse.length > 0) {
          ...
          let title =
            customerPoints > 0 ? `أهلاً بعودتك، هل أنت محتار؟` : `محتار ماذا تختار؟`;
          if (customerName) {
            title = `${customerName}، هل أنت محتار اليوم؟`;
          }

          setPsychMessage({
            title,
            desc: `أغلب عملائنا أحبوا ${suggestion.name}، متوفر الآن ويمكنك طلبه.`,
  ...
  const moodResponses = useMemo(() => ({
    gathering: [
      "لا تقلق يا أخي، صوانينا مميزة وستصلك ساخنة! نحن هنا لخدمتك دائماً 🚀",
      "هل استقبلت ضيوفاً فجأة؟ ابقَ مرتاحاً، صوانينا الكبيرة ستصلك لتجمل ضيافتك أمام أصدقائك 😎",
      "لمناسباتك واجتماعاتك.. لا تشغل بالك بالطعام، الطاهي جهز لك صواني مميزة ترفع الرأس 👑"
    ],
    sick: [
      "شفاها الله وعافاها من كل مكروه.. لقد أعددنا لكِ حساء الخضار العطرة التقليدي الدافئ الذي يمنح الراحة والدفء، تمنياتنا لها بالشفاء العاجل والخير 🤍🍲",
      "حمدًا لله على سلامتها وعافيتها 🌿.. لقد أعد لكِ الطاهي حساء لسان عصفور بالهيل دافئاً وخفيفاً يمنح الجسم النشاط والدفء، تمنياتنا لها بالشفاء العاجل 🥣🤍",
      "أتمنى لكم الصحة والعافية الدائمة.. هذه أنواع الحساء الأصيلة المطبوخة على نار هادئة والممزوجة بالخضار العطرة، وضعناها لكِ في القائمة لتمنح الراحة والنشاط، بالشفاء التام إن شاء الله 🍵✨"
    ],
    travel: [
    ...
    sad: [
      "روّق مزاجك! لا شيء يستحق الحزن، وهذه الحلوى اللذيذة ستسعد يومك وتعدل مزاجك 🍫✨",
      "الحياة لا تستحق حزنك.. اطلب الآن من تشكيلة الحلوى المميزة التي تسعد قلبك وتفتح نفسك للبهجة 🍰",
      "أتشعر ببعض الضيق؟ صدقني، القليل من الكاكاو والحلويات كفيل بتحويل يومك إلى بهجة وسرور.. دلل نفسك 🎂"
    ],
    late: [
      "هل تسهر بمفردك؟ دعنا نعتني بك بهذه الطلبات اللذيذة التي تنسيك تعب اليوم 🌙🍛",
      "جوع آخر الليل يحتاج حلاً سريعاً.. اطلب ما تشتهيه الآن وأكمل سهرتك بسعادة 🍲",
      "ما الذي يسهرك حتى هذا الوقت؟ أتشعر بالجوع؟ الطاهي جاهز الآن ويعد لك أفضل طلب! 🥘"
    ],
    general: [
      "اطلب ما تشاء.. الطاهي تحت خدمتك اليوم! 👨‍🍳",
      "ما الذي تفضله اليوم؟ اكتب ما تشتهيه وسنحضره لك بكل سرور! 😋",
      "تفضل واطلب ما تحب.. قائمة الطعام كاملة من أجلك! أخبرني، ماذا تود أن تأكل؟ 🍽️"
    ]
  }), []);
...
\u2709\uFE0F *ملاحظات عامة:* ${order.generalNotes}`;
  ...
  const themeContext = useMemo(() => {
    ...
    if (hour >= 22 || hour < 3) {
      ...
      const title = getRand([
        "هل تشعر بالجوع في وقت متأخر؟ 🌙",
        "لست وحدك في سهرتك.. 🌙",
      ]);

      let desc = "";
      if (product.includes("مجبوس") || product.includes("لحم")) {
        desc = `تسهر بمفردك؟ دعنا نعتني بك ونقدم لك هذا الطبق الشهي (${product}) الذي ينسيك تعب اليوم كله ويمنحك الراحة.`;
      } else {
        desc = `نعلم أن سهراتك تحتاج إلى وجبات خفيفة.. لهذا أعددنا لك ${product} بما يناسب مزاجك ولا يثقل نومك!`;
      }
                ...
                <div className="flex items-center gap-3.5 min-w-0 flex-1">
                  ...
                  <div className="text-right min-w-0 flex-1 space-y-1">
                    ...
                    <p className="text-xs sm:text-sm font-black text-brand line-clamp-1 group-hover:text-amber-950 transition-colors">
                      ما رأيك أن نكملها مع <span className="text-amber-800 font-extrabold">{suggestion.name}</span>؟
                    </p>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              {productAddons.filter((a: any) => getQuantityRuleLimits(a, quantity).available && !selectedAddonsIds.includes(getAddonKey(a))).slice(0, 1).map((recommendedAddon: any) => {
                const creativeMessages = [
                  ...
                  // عبارات فيها تفاعل
                  `ينقصك شيء واحد لتكتمل التجربة.. "${recommendedAddon.name}"! 😉`,
                  `ما رأيك أن تكملها وتضيف "${recommendedAddon.name}"؟ لن تندم أبداً 😍`,
                  ...
                  // عبارات سريعة ومرحة
                  `تريد أن تدلل نفسك؟ أضف "${recommendedAddon.name}" واستمتع 😎`,
                  `لماذا تتناوله عادياً بينما يمكنك جعله أسطورياً مع "${recommendedAddon.name}"؟ 🚀`,
