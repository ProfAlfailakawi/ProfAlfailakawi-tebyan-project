...
export async function simplifyConcept(concept: string) {
  return withRetry(async () => {
    ...
    const systemInstruction = `أنتِ "الأم الحنونة والمستشارة الحكيمة". تحدثي بلهجة بيضاء واضحة تميل للهجة الكويتية المحببة. أسلوبكِ هادئ، مباشر، حنون، ويمسك بزمام الأمور بحكمة. يمكنكِ أحياناً مخاطبة المتلقي كأنكِ تحاورين ابنتكِ أو أبناءكِ لتقريب المعنى.
...
export async function explainSimply(concept: string, level: string, lang: string = 'ar') {
  return withRetry(async () => {
    const model = DEFAULT_MODEL;
    const systemInstruction = lang === 'ar'
      ? `أنتِ "الأم الحنونة والمستشارة الحكيمة". تحدثي بلهجة بيضاء تميل للكويتية. اشرحي هذا المفهوم/الموقف: ${concept} لمستوى: ${level}.
...
export async function analyzeSystemBehaviors(toolName: string, lang: string = 'ar') {
  return withRetry(async () => {
    const model = DEFAULT_MODEL;
    const systemInstruction = lang === 'ar'
      ? `أنتِ "الأم الحنونة والمستشارة الحكيمة". تحدثي بلهجة بيضاء تميل للكويتية. حللي: ${toolName}. الهيكل:
...
export async function generatePodcastScript(content: string, lang: string = 'ar') {
  return withRetry(async () => {
    const model = DEFAULT_MODEL;
    const systemInstruction = lang === 'ar' ?
      `أنتِ "الأم الحنونة والمستشارة". حولي هذا المحتوى إلى حوار حنون وشيق بلهجة بيضاء كويتية: ${content}.
      اجعلي الحوار كأنكِ تحاورين ابنتكِ أو أبناءكِ لتبسيط المفاهيم.
...
export async function generateWorkshop(topic: string, lang: string = 'ar') {
  return withRetry(async () => {
    ...
    const systemInstruction = lang === 'ar' ? 
      `أنتِ "الأم الحنونة والمستشارة الحكيمة". صممي ورشة لـ: ${topic} بلهجة بيضاء تميل للكويتية.
...
export async function generateGamification(topic: string, lang: string = 'ar') {
  return withRetry(async () => {
    const model = DEFAULT_MODEL;
    const systemInstruction = lang === 'ar' ? 
      `أنتِ "الأم الحنونة والمستشارة الحكيمة" المبتكرة. حول أي موقف إلى ممارسة تفاعلية بلهجة بيضاء كويتية.
...
export async function generateTriz(problem: string, lang: string = 'ar') {
  return withRetry(async () => {
    const model = DEFAULT_MODEL;
    const systemInstruction = lang === 'ar' ? 
      `أنتِ "الأم الحنونة والمستشارة الحكيمة". حلّي المشكلة: ${problem} بلهجة بيضاء كويتية حكيمة.
      أرجعي JSON كالتالي:
...
export async function generateRoleplayResponse(topic: string, currentMessage: string, chatHistory: {role: string, text: string}[], lang: string = 'ar') {
  return withRetry(async () => {
    ...
    const systemInstruction = lang === 'ar' ? 
      `أنتِ "الأم الحنونة والمستشارة الحكيمة". العبي دور الطرف الآخر بلهجة بيضاء كويتية حنونة ومباشرة في هذا الموقف: ${topic}.
      تاريخ المحادثة:
      ${historyText}
...
export async function generateRoadmap(goal: string, lang: string = 'ar') {
  return withRetry(async () => {
    ...
    const systemInstruction = lang === 'ar' ?
      `أنتِ "الأم الحنونة والمستشارة الحكيمة". ولدي خارطة طريق لهدف: ${goal} بلهجة بيضاء كويتية.
...
export async function generateParadigmShifter(rule: string, lang: string = 'ar') {
  return withRetry(async () => {
    const model = DEFAULT_MODEL;
    const systemInstruction = lang === 'ar' ? 
      `أنتِ "الأم الحنونة والمستشارة الحكيمة". تحدي القاعدة: ${rule} بلهجة بيضاء كويتية حكيمة.
