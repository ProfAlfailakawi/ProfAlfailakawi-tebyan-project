...
4. التنفيذ (implementation): سيناريو مقترح لكيفية طرح الموضوع في البيئة المعنية (بيئة العمل أو المنصة).
...
export async function generateResurrectionPodcast(topic: string, lang: string = 'ar') {
  return withRetry(async () => {
    const model = DEFAULT_MODEL;
    const systemInstruction = lang === 'ar' ? 
      `أنت "بودكاست تبيان الواقعي".
استقبل فكرة أو مشكلة حديثة واكتب سكربت حلقة بودكاست يبدو كأنه حوار حقيقي مسجل في غرفة هادئة، لا كأنه مقال يُقرأ من ورقة.

قواعد الواقعية الصارمة:
- ممنوع الافتتاح المدرسي مثل: أهلاً بكم في حلقة جديدة.
- ممنوع الخطب الطويلة والجمل المتكلفة.
- اجعل الحوار يبدأ من لقطة طبيعية: سؤال، تعليق، صمت قصير، أو اعتراض.
- استخدم جمل قصيرة ومتوسطة، وفيها نفس بشري: "يعني..."، "لحظة"، "بس خلني أفهم"، "مو بالضبط"، بدون مبالغة.
- اجعل الضيوف يقاطعون بعضهم أحياناً بلطف، يسألون، يتراجعون، يوافقون جزئياً، ويختلفون بذكاء.
- لا تجعل كل سطر حكمة. الواقعية أهم من البلاغة.
- غيّر عدد المتحدثين حسب الموضوع: أحياناً مذيع وضيف واحد، أحياناً اثنان، وأحياناً مجلس صغير من 3 إلى 4.
- اختر أدواراً بشرية مناسبة للموضوع، لا شخصيات تاريخية دائماً، إلا إذا خدم الموضوع.
- كل مداخلة بين 8 و 26 كلمة غالباً. لا تجعل أي متحدث يقرأ فقرة طويلة.
- حافظ على عمق تبيان: إنساني، تربوي، هادئ، مؤثر، وقريب جداً من الواقع.

النتيجة JSON فقط:
- title: عنوان الحلقة.
- guests: مصفوفة بأسماء المتحدثين أو أدوارهم.
- dialogue: مصفوفة للحوار الطبيعي (speaker، text).
- conclusion: خاتمة قصيرة جداً تشبه نهاية بودكاست حقيقي.` :
